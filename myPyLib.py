...
def printResult(xxx, count):
    for i in range(count):
        print("ID = %s  ||  TWT = %.2f  || CO2=%.2f   || CO=%.2f  || NOx =%.2f || distance=%.2f " % (
            xxx[i][0], xxx[i][1], xxx[i][2], xxx[i][3], xxx[i][4], xxx[i][5]))
...
def StopAtRoadSide(veh, endpos, startpos):
    ...
    if (x1 == 10 and edge == "--101938#11"):
        traci.vehicle.setStop(veh, "--101938#11", endpos,
                              0, 10000, 1, startpos, -1)
    if (x2 == 10 and edge == "--101932"):
        traci.vehicle.setStop(veh, "--101932", endpos,
                              0, 10000, 1, startpos, -1)
