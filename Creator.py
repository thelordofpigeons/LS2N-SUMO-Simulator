...
def injectionTraffic(mapName,nbTrucks,vehicle_type):
    ...
    dom = minidom.parseString(ET.tostring(outRoot, encoding="unicode", method=None))  
...
def createMissions(mapName,nbTrucks, traffic):
    ...
    dom = minidom.parseString(ET.tostring(missionRoot, encoding="unicode"))  # , parser).parse("data/output.xml") # or xml.dom.minidom.parseString(xml_string)
...
if __name__ == "__main__":
    create(mapName, 100)   # or ask user input later
