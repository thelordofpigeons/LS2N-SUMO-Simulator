...
def load_settings():
    ...
    try:
        ...
        elif map_list and map_list[0] != "No Maps Found":
        ...
        saved_infra_only = settings.get(
            "infra_only", False)  # Default to False
        var_infra_only.set(bool(saved_infra_only))  # Ensure it's a boolean

        # Load Launch Mode
        saved_launch_mode = settings.get(
            "launch_mode", "Mode111")  # Default mode
        ...
        print(
            f"Error loading or parsing {CONFIG_FILE}: {e}. Using default settings.")
    except Exception as e:
        print(
            f"An unexpected error occurred while loading settings: {e}. Using default settings.")
        ...
        for item in os.listdir(maps_dir):
            ...
            if os.path.isdir(item_path):
                ...
                network_file = os.path.join(
                    item_path, "MyNetwork.net.xml")  # Or Network.sumocfg
                if os.path.exists(network_file):
                    # Use directory name as map name
                    available_maps.append(item)
    if not available_maps:
        messagebox.showerror(
            "Error", "No map directories found in 'cases/' folder or they lack necessary files.")
...
def style_widgets():
    ...
    style.configure("Card.TLabelframe.Label", font=(
        'Segoe UI', 11, 'bold'), background="#ffffff", foreground="#2c3e50")

    style.configure("TFrame", background="#ffffff")
    style.configure("TLabel", background="#ffffff",
                    foreground="#2c3e50", font=('Segoe UI', 10))
    style.configure("TEntry", font=('Segoe UI', 10))
    style.configure("TCombobox", font=('Segoe UI', 10))
    style.configure("TButton", font=('Segoe UI', 10, 'bold'), padding=8)
    style.configure("TProgressbar", background="#e0e0e0",
                    troughcolor="#ffffff")  # Style progress bar
...
def create_instance():
    ...
    if var_infra_only.get():
        ...
        try:
            ...
            messagebox.showerror(
                "Invalid Input", "Please enter a valid non-negative number of trucks.")
            return
    ...
    log_message(
        f"Creating instance for '{map_name}' with {truck_count} {vehicle_type}(s)...")
    ...
    window.after(100, _check_creator_thread,
                 creator_thread, map_name, truck_count)
...
def _check_creator_thread(thread, map_name, truck_count):
    ...
    if thread.is_alive():
        ...
        if window.creator_result["success"]:
            ...
            log_message(
                f"ERROR: {window.creator_result['message']}{error_details}")
            # Keep popup for errors
            messagebox.showerror(
                "Error", window.creator_result["message"] + error_details)
...
def launch_instance():
    ...
    if map_name == "No Maps Found" or not selected_map:
        log_message(
            "Please select a valid map in the 'Launch Simulation' section first.")
        messagebox.showwarning("Missing selection",
                               "Please select a valid map.")
        return
    ...
    log_message(
        f"Initiating simulation process for {map_name} (Mode: {launch_mode})...")
    ...
    starter_thread = threading.Thread(
        ...
        # Allows main GUI to exit even if sim thread has issues (can be debated)
        daemon=True
    )
    ...
    messagebox.showinfo(
        "Process Started", f"Simulation process for '{map_name}' initiated.\nA separate monitor window should appear shortly.")
...
main_frame.pack(pady=15, padx=10, fill=BOTH,
                expand=True)  # Allow frame to expand
...
create_frame = ttk.LabelFrame(
    main_frame, text="🛠 Create Instance", style="Card.TLabelframe", padding=20)
# Use sticky='nsew' to make it fill the grid cell
create_frame.grid(row=1, column=0, padx=30, pady=10,
                  ipadx=5, ipady=5, sticky='nsew')

ttk.Label(create_frame, text="Map Selection:").pack(anchor=W, padx=5)
CHX_Create = ttk.Combobox(create_frame, values=map_list,
                          width=28, state="readonly")
if map_list and map_list[0] != "No Maps Found":
    CHX_Create.current(0)  # Select first map if available
CHX_Create.pack(pady=5, fill=X, padx=5)  # Fill available width


# Frame to choose the number of trucks
ttk.Label(create_frame, text="Number of Trucks:").pack(
...
ttk.Label(create_frame, text="Vehicle Type:").pack(
    anchor=W, pady=(10, 0), padx=5)
CHX_VehType = ttk.Combobox(create_frame, values=[
                           "Truck", "MissionVehicle"], width=28, state="readonly")
...
launch_frame = ttk.LabelFrame(
    main_frame, text="🚦 Launch Simulation", style="Card.TLabelframe", padding=20)
launch_frame.grid(row=1, column=1, padx=30, pady=10,
                  ipadx=5, ipady=5, sticky='nsew')
launch_frame.pack_propagate(False)

ttk.Label(launch_frame, text="Map Selection:").pack(anchor=W, padx=5)
CHX_Launch = ttk.Combobox(launch_frame, values=map_list,
                          width=28, state="readonly")
if map_list and map_list[0] != "No Maps Found":
    CHX_Launch.current(0)  # Select first map if available
CHX_Launch.pack(pady=5, fill=X, padx=5)

# --- START: Add Launch Mode Input ---
ttk.Label(launch_frame, text="Launch Mode:").pack(
    anchor=W, pady=(10, 0), padx=5)
var_launch_mode = StringVar(value="Mode111")  # Default value
entry_launch_mode = ttk.Entry(
    launch_frame, textvariable=var_launch_mode, width=30)
